...
train_data = torchvision.datasets.ImageFolder(root=r"D:\pycharm\project\Spatiotemporal Analysis\water depth prediction2.0\dataset\train-2", transform=data_transform["train"])

traindata = DataLoader(dataset=train_data, batch_size=128, shuffle=True, num_workers=0)  # 将训练数据以每次128张图片的形式抽出进行训练

test_data = torchvision.datasets.ImageFolder(root=r"D:\pycharm\project\Spatiotemporal Analysis\water depth prediction2.0\dataset\test-2", transform=data_transform["val"])

testdata = DataLoader(dataset=test_data, batch_size=128, shuffle=True, num_workers=0)  # 将训练数据以每次128张图片的形式抽出进行测试
...
def vit_base_patch16_224(num_classes: int = 1000):
    ...
    ImageNet-1k ViT_weights @ 224x224, source https://github.com/google-research/vision_transformer.
    ViT_weights ported from official Google JAX impl:
...
def vit_base_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k ViT_weights @ 224x224, source https://github.com/google-research/vision_transformer.
    ViT_weights ported from official Google JAX impl:
...
def vit_huge_patch14_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    ImageNet-21k ViT_weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: converted ViT_weights not currently available, too large for github release hosting.
    """
    model = VisionTransformer(img_size=224,
                              patch_size=14,
                              embed_dim=1280,
