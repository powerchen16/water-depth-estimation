...
class SEBlock(nn.Module):                      #先创建SENet模块
...
for epoch in range(100):
    ...
    # save model
    torch.save(model.state_dict(), "resnet50-SE-last.pt")
    if acc_val == max(val_acc_list):
        torch.save(model.state_dict(), "resnet50-SE-best.pt")
...
plt.savefig(r'D:\qianmengchen\water depth prediction\Resnet-classification\结果分析图\resnet-SENet-512-1e-4-数增-1e-4(2).png')
plt.show()
